...
if (!preg_match($float, $p)) {
    $erro[5] = "O preço à vista deve ter a parte dos centavos <br />";
} else {
...
if (!preg_match($float, $prazo)) {
    $erro[7] = "O preço à prazo deve ser informado e ter a parte dos centavos <br />";
} else {
...
if (count($erro) != 0) {
    ...
    if (!$incluir) {
        ...
        //inclui a miniatura
        if ($foto_m != null) {
            $incluir = $imagem->incluirImg($conexao, $foto_m, $id, 1, 1);
            if (!$incluir) {
        ...
        while ($i < 8) {
            if ($foto[$i]["name"] != null) {
                $incluir = $imagem->incluirImg($conexao, $foto[$i], $id, 1, 2);
                if (!$incluir) {
