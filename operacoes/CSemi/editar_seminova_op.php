...
if (count($erro) != 0) {
    ...
    $incluir = $moto->editarSemi($id, $marca, $modelo, $ano, $cilindradas, $transmissao, $partida, $refrigeracao, $esa, $ascs, $rdc, $bc, pg_escape_string(htmlspecialchars(stripslashes($observacao))), $bolsas, $motor, $potencia, $cor, $km, $estilo, $preco);
    ...
    if (!$incluir) {
        ...
        //inclui a miniatura
        if ($foto_m != null) {
            $incluir = $imagem->incluirImgSemiNovas($conexao, $foto_m, $id, 3, 1);
            if (!$incluir) {
        ...
        while ($i < 8) {
            if ($foto[$i]["name"] != null) {
                $incluir = $imagem->incluirImgSemiNovas($conexao, $foto[$i], $id, 3, 2);
                if (!$incluir) {
